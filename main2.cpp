...
int fitness(const std::vector<Task> &tasks, const std::vector<Machine> &machines,
  ...
  for (int i = 0; i < tasks.size(); i++) {
    ...
    if (std::find(tasks[task_id].affinitive_machines.begin(), tasks[task_id].affinitive_machines.end(), machine_id) == tasks[task_id].affinitive_machines.end()) {
      std::cerr << "Task " << task_id + 1 << " is not assigned to its affinitive machine " << machine_id + 1 << std::endl;
  ...
  for (int i = 0; i < disks.size(); i++) {
    if (disk_data_usage[i] > disks[i].quota) {
      std::cerr << "Disk " << i + 1 << " quota exceeded." << std::endl;
  ...
  for (int m = 0; m < machines.size(); m++) {
    ...
    for (int i = 1; i < intervals.size(); i++) {
      if (intervals[i].first < intervals[i - 1].second) {
        std::cerr << "Machine " << m + 1 << " has overlapping tasks." << std::endl;
        return INT_MAX; // 任务重叠
      }
    }
  }

  std::cerr << "Fitness evaluation complete: Makespan = " << makespan << std::endl;
  return makespan;
}
